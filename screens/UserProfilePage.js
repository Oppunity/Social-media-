...
async componentWillMount(){
 ...
 this.setState({sUser:this.props.navigation.getParam('sUser'),followingright: '1%',Default_Rating:0,smash:0,heat:0,looseness:0,thickness:0,intelligence:0,savageness:0,ratchetness:0,diversity:0}, async function(){
    //console.log('sUser.... '+this.props.navigation.getParam('sUser').identityId)
    console.log('sUseruser.... '+this.state.sUser.userName)
    console.log('sUseruser.... '+this.state.sUser.orgName)
...
if(orgquery.data.listOrgInfos.items.length>0){
    ...
    var AllUserData = await API.graphql(graphqlOperation(queries.listOrgEvents, { filter: { identityId: { eq: this.state.IDENTITYID } } } ))
    var userpic = await API.graphql(graphqlOperation(queries.listUserPics, { filter: { identityId: { eq: this.state.IDENTITYID } } } ))
    //const userinfo = await API.graphql(graphqlOperation(listUserInfos, { filter: { identityId: { eq: this.state.IDENTITYID } } } ))
    ...
    userpic: userpic.data.listUserPics.items[0],
    userinfo: this.state.sUser,
    ...
    orgtext:this.state.sUser.userBio,
    }) 
    //console.log('userinfo1: ', userinfo)
    //console.log('currentUser: ',this.state.currentUser)
    for(let i=0; i<this.state.currentUser.length; i++) {
    this.setState({userpicname: [...this.state.userpicname, this.state.currentUser[i].picpath.split('/').slice(-1)[0]]})
    } 

        //console.log('orgpicname: ', this.state.userpicname)
    } catch (err) {
        console.log('error fetching currentUser...', err)
    }

    //console.log('orgpic.OrgPicture:', this.state.userpic.UserPicture)
    await Storage.get(this.state.userpic.UserPicture, {level: 'protected'})
    .then(data => {
    this.setState({
    userimage: data
    })
    ...
    for(let i=0; i<this.state.userpicname.length; i++) {
        await Storage.get(this.state.userpicname[i], {level: 'protected'})
        .then(data => {
 ...
 this.willFocusSubscription1 = this.props.navigation.addListener('didFocus', async(payload) => {
...
/*onNavigatorEvent(event) {
    if (event.id === 'backPress') {
        ...
        this.props.navigation.navigate('FollowerPage',{sUser:this.state.userinfo })
    }
}*/

componentWillUnmount() {
    //if(this.props.navigation.state.params.onGoBack){
    //    console.log('made it to component will unmount '+this.state.userinfo)
      this.props.navigation.state.params.onGoBack(this.state.userinfo);
      this.willFocusSubscription1.remove()
      //this.willFocusSubscription.remove();
   // } 
  }
 onGoBack = (someDataFromModal) => {
    this.setState({sUser:this.state.userinfo}) 
    console.log('on go back user profile page worked')
...
updateFollow = async() => {
    ...
    this.setState({followings:true,backgroundColor: 'black', borderColor: 'grey',buttonMessage: 'Following',followerrowid:cUserfollows.data.listFollowers.items[0].id,followingrowid:cUserfollowing.data.listFollowings.items[0].id})
        ...
        //await API.graphql(graphqlOperation(mutations.deleteFollowing, {input: {Followerid:this.state.CurrentIDENTITYID, Followingid:this.state.orginfo.identityId,Followingname:this.state.orginfo.orgName,Followingppic:this.state.orginfo.OrgPicture} }))
   //await API.graphql(graphqlOperation(mutations.deleteFollower, {input: {Followingid:this.state.orginfo.identityId, Followerid:this.state.CurrentIDENTITYID,Followername:this.state.cUsername,Followerppic:this.state.cUserpic} }))
   ...
   following:following})
    ...
    await API.graphql(graphqlOperation(mutations.createFollowing, {input: {Followerid:this.state.CurrentIDENTITYID, Followingid:this.state.userinfo.identityId,Followingname:this.state.userinfo.userName,Followingppic:this.state.userinfo.UserPicture} }))
    await API.graphql(graphqlOperation(mutations.createFollower, {input: {Followingid:this.state.userinfo.identityId, Followerid:this.state.CurrentIDENTITYID,Followername:this.state.cUsername,Followerppic:this.state.cUserpic} }))
    var followers = await API.graphql(graphqlOperation(queries.listFollowers, { filter: { Followingid: { eq: this.state.IDENTITYID } } } ))
   var following = await API.graphql(graphqlOperation(queries.listFollowings, { filter: { Followerid: { eq: this.state.IDENTITYID } } } ))
...
renderSectionFourth = () => {
 ...
 <View style={{flexDirection: 'row',}} > 
 <Text style={{marginLeft: 250, marginTop: 20, fontWeight: '900', color: '#42a5f5', fontSize: 25}}>{this.state.userinfo.userName}</Text>
 </View>

 <TouchableOpacity style={{position:'absolute', top: '33%', right: '48%'}} onPress={() => {console.log('userName being passed'+this.state.userinfo.userName);this.props.navigation.navigate('FollowerPage',{sUser:this.state.userinfo,onGoBack:this.onGoBack })}}> 
 {/*
...
*/}
 ...
 <TouchableOpacity style={{position:'absolute',top: '33%', right: '15%'}} onPress={() => {console.log('userName being passed'+this.state.userinfo.userName);this.props.navigation.navigate('FollowingPage',{sUser:this.state.userinfo,onGoBack:this.onGoBack })}}>
 ...
 <Text style={{ color: 'white'}}> {this.state.orgtext} </Text> 
 </View>
...
export default withNavigation(UserProfilePage); 